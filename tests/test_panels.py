...
- The yt_center panel MUST be declared with center_overlay=True. Every
  channel-click flows through ui.Call("__panel__yt_center2", channel_id=...)
...
def test_yt_center_panel_is_declared_as_center_overlay():
    ...
    ui.Call("__panel__yt_center2", channel_id=...). The SDK only gives a
    ...
    (yt_connect_dialog, yt_settings2) already carries the flag; yt_center2
    must too.

    NOTE: this panel was renamed yt_center -> yt_center2 (and
    yt_settings -> yt_settings2) as a platform-side workaround: the
    browser console kept logging "yt_center -> right" / "yt_settings ->
    right" instead of "-> center" despite the manifest/code being
    correct on every redeploy, incognito session, and version bump --
    pointing at a stuck platform-side slot registration keyed by the old
    panel_id. Renaming forces a fresh registration under a new id."""
    panel_meta = ext.panels.get("yt_center2")
    assert panel_meta is not None, "yt_center2 panel is not registered"
    assert panel_meta.get("slot") == "center"
    assert panel_meta.get("center_overlay") is True, (
        "yt_center2 must be declared with center_overlay=True or channel "
